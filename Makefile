...
LFLAGS=$(WALL) $(LIBJLIB2) -o jbold $(LIBS)

ALL_OBJ=jbold$(OBJ) jbold_e$(OBJ) jbold_h$(OBJ) jbold_i$(OBJ) jbold_l$(OBJ)
ALL_C=jbold.c jbold_e.c jbold_h.c jbold_i.c jbold_l.c
ALL_H=jbold.h

#
# do the work
#
all:	ckenv tags $(ALL_OBJ) jbold.1$(GZ)
...
install:	all
	...
	-$(CHMOD) 755 $(PRODMAN)
	-$(STRIP) jbold
	$(CP) jbold $(PRODUCTION)/jbold
	$(CHMOD) 755 $(PRODUCTION)/jbold
	$(CP) jbold.1$(GZ) $(PRODMAN)/jbold.1$(GZ)
	$(CHMOD) 644 $(PRODMAN)/jbold.1$(GZ)

uninstall:	all
	-$(RM) $(PRODUCTION)/jbold
	-$(RM) $(PRODMAN)/jbold.1.gz
	-$(RM) $(PRODMAN)/jbold.1
